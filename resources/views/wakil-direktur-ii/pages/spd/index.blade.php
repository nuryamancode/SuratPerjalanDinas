                                        ...
                                        <td>
                                            ...
                                            @foreach ($item->spj_many as $spj)
                                                @if ($spj->pembuat_spj == $userId)
                                                    ...
                                                    <a href="{{ route('wakil-direktur-ii.spd-spj.show', $spj->id) }}"
                                                        class="btn btn-info py-2">Lihat SPJ</a>
                                                @break
                                            @endif
                                        @endforeach
                                        @if (!$isPembuatSpj)
                                            <a href="{{ route('wakil-direktur-ii.spd-spj.create', ['spd_id' => $item->id]) }}"
                                                class="btn btn-primary py-2">Buat SPJ</a>
                                        @endif
                                        <a href="{{ route('wakil-direktur-ii.spd.show', $item->id) }}"
