                            ...
                            <tbody>
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>
                                            {{-- <a href="{{ route('wakil-direktur-ii.spd-detail-supir.index', [
                                                'spd_detail_uuid' => $item->uuid,
                                            ]) }}"
                                                class="btn btn-primary  py-2">Supir</a>
                                            <a href="{{ route('wakil-direktur-ii.spd-detail.edit', $item->uuid) }}"
                                                class="btn btn-sm py-2 btn-info">Edit</a> --}}
                                            @if ($item->spj)
                                                <a href="{{ route('wakil-direktur-ii.spd-spj.show', $item->spj->uuid) }}"
                                                    class="btn btn-info  py-2">Lihat SPJ</a>
                                            @else
                                                <a href="{{ route('wakil-direktur-ii.spd-spj.create', [
                                                    'spd_detail_uuid' => $item->uuid,
                                                ]) }}"
