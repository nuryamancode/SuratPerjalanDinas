                    ...
                    <form action="{{ route('karyawan.update', $item->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class='form-group'>
                            ...
                            <select name='golongan_id' id='golongan_id'
                                ...
                                @foreach ($data_golongan as $golongan)
                                    <option @selected($golongan->id == $item->golongan_id) value='{{ $golongan->id }}'>{{ $golongan->nama }}
                                    </option>
                                @endforeach
                            </select>
                            @error('golongan_id')
                        ...
                        <div class='form-group'>
                            ...
                            <select name='status_akun' id='status_akun'
                                ...
                                <option @selected(!$item->user) value='0'>Tanpa Akun</option>
                                <option @selected($item->user) value='1'>Dengan Akun</option>
                            </select>
                            @error('status_akun')
                                <div class='invalid-feedback'>
                                    {{ $message }}
                        ...
                        <div class="akun_user @if (!$item->user) d-none @endif">
                            <div class='form-group mb-3'>
                            ...
                            <div class='form-group mb-3'>
                                <label for='password_confirmation' class='mb-2'>Konfirmasi Password (Opsional)</label>
                            ...
                            <div class='form-group'>
                                ...
                                <select name='role' id='role'
                                    ...
                                    @foreach ($data_role as $role)
                                        <option @selected($role->name == $item->user && $item->user->getRoleNames()->first() ?? null) value='{{ $role->name }}'>
                                            {{ $role->name }}
                                        </option>
                                    @endforeach
                                </select>
